...
import { ConsoleReporter, MatrixReporter, methodOf, Report, Reporter } from "./reporters"
...
function methodFilter(method: string, sub: MethodSubscription): boolean {
	if (Object.keys(sub).includes("only")) {
		...
		logger.warn('Config "method_subscription" missing or invalid, defaulting to "all".');
		return true
	}
}
...
async function listAllChains(config: AppConfig, reporters: Reporter[]) {
	const _ = await Promise.all(
        config.endpoints.map(
            (e) => listenChain(e, config.listen, config.accounts, config.method_subscription, reporters)
        )
    );
...
async function main() {
	if (!argv.c) {
		logger.error('-c or APP_CONFIG_FILE env variable must specify a config file');
		process.exit(1);
	}
	...
	if (config.listen !== ApiSubscription.Finalized && config.listen !== ApiSubscription.Head) {
		logger.warn(`"listen" config not provided or invalid: (${config.listen}), defaulting to "${ApiSubscription.Finalized}"`);
